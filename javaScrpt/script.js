...
addFav.forEach(boton => {
    if (JSON.parse(localStorage.getItem("Favoritos")).find(item => item.rank == boton.id)) {
        boton.innerHTML = `<i class="fa-solid fa-heart"></i>`
    }

    boton.addEventListener("click", (e) => {
        imdbTop250.map(valor => {
            if (valor.rank == boton.id) {

                if (!JSON.parse(localStorage.getItem("Favoritos")).find(item => item.rank == boton.id) || localStorage.getItem("Favoritos").length == 0) {
                    boton.innerHTML = `<i class="fa-solid fa-heart"></i>`
                    ...
                    let newListaFav = JSON.parse(localStorage.getItem("Favoritos"))
                    // newListaFav.pop(newListaFav.indexOf(valor))
                    newListaFav = newListaFav.filter(item => console.log(item.rank !== boton.id))
